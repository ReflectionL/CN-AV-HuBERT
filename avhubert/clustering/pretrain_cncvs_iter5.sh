...
do echo $split;
    ...
    #     ${tsv_dir} \
    #     $split \
    #     ${model_root}/clean_pt_cncvs_speech_iter4/checkpoints/checkpoint_best.pt \
    #     12 \
    #     4 \
    #     ${i} \
    #     ${feature_dir}/iter4_L12/ \
    #     --user_dir `pwd`/../ &
    # done
...
for rank in $(seq 0 3); do
    cat ${label_dir}iter4_L12/train_${rank}_4.km
done > ${label_dir}iter4_L12/train.km
for rank in $(seq 0 3); do
    cat ${label_dir}iter4_L12/valid_${rank}_4.km
done > ${label_dir}iter4_L12/valid.km
for rank in $(seq 0 3); do
    cat ${label_dir}iter4_L12/test_${rank}_4.km
done > ${label_dir}iter4_L12/test.km

## generate dict.km.txt
for i in $(seq 0 1999);do
    echo $i 10000
done > ${label_dir}/iter4_L12/dict.km.txt
